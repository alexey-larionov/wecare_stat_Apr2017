---
title: "explore_wecare_vs_nfe_3ev_nfe_Mar2017"
...
# Load data
load(paste(interim_data_folder, "r08c_wecare_vs_nfe_skat_3ev_nfe_Mar2017.RData", sep="/"))
...
sel_genes.df <- cbind(sel_genes.df, 
                      p_lrt_nfe=result_glm.df[result_skat.df$gene,"p_lrt_nfe"],
                      p_lrt_kgen50=result_glm.df[result_skat.df$gene,"p_lrt_kgen50"])

# Plot lrt vs burden ...
plot(p_lrt_nfe~svt_burden_p, data=sel_genes.df)

# p vs counts
plot(p_lrt_nfe~counts_per_gene, data=sel_genes.df)
...
qqunif.plot(sel_genes.df$p_lrt_nfe[sel_counts & sel_genes.df$p_lrt_nfe != 0], 
              main=paste("wecare vs nfe, 3ev nfe, LRT, counts > ", min_counts, "\n(", sum(sel_counts), " genes x 676 cases)", sep=""))
...
x <- sel_genes.df[sel_counts & sel_genes.df$p_lrt_nfe != 0,]
x <- x[order(x$p_lrt_nfe),]
x[1:20,]

print("TRBV10-1 = T cell receptor beta variable 10-2")
...
qqunif.plot(sel_genes.df$svt_burden_p[sel_counts & sel_genes.df$svt_burden_p != 0], 
              main=paste("wecare vs nfe, 3ev nfe, svt+burden, counts > ", min_counts, "\n(", sum(sel_counts), " genes x 676 cases)", sep=""))
...
qqunif.plot(sel_genes.df$svt_skat_p[sel_counts & sel_genes.df$svt_skat_p != 0], 
              main=paste("wecare vs nfe, 3ev nfe, svt+skat, counts > ", min_counts, "\n(", sum(sel_counts), " genes x 676 cases)", sep=""))
