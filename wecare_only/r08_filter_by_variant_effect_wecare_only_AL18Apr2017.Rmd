...
last updated: Alexey Larionov, 18Apr2017
...
write.table(y, file=paste(results_folder, "wecare_variants_per_gene.txt", sep="/"), quote = FALSE, row.names = FALSE)

# Plot histogram
z <- variants.df %>%  group_by(SYMBOL) %>% summarise(n())
"vars_count" -> colnames(z)[2]
hist(z$vars_count, main="Histogram for numbers of protein-affecting variants per gene (wecare-only)", xlab="Variants per gene", ylab="Number of genes", ylim=c(0,8000), breaks=25, labels = TRUE)
mean(z$vars_count) # ~2.0
...
genes.mx <- matrix(ncol=ncol(genotypes.mx),nrow=0)
...
# Data frame with aggregated counts per gene for all cases
genes.df <- data.frame(genes, counts_per_gene)
genes.df$genes <- as.character(as.vector(genes.df$genes))
genes -> rownames(genes.df)

str(genes.df)
genes.df[1:5,]
genes.df[genes.df$genes=="ATM",]
genes.df[genes.df$genes=="CHEK2",]
...
hist(counts_per_gene, labels = TRUE, ylim=c(0,10000), 
     xlab="aggregated counts per gene (bins)", ylab="num of genes")

hist(counts_per_gene[counts_per_gene<26], breaks=c(0:25), 
     labels = TRUE, ylim=c(0,4000), main="zoom to <25 alleles (per 478 cases)", 
     xlab="aggregated count per gene", ylab="num of genes")$counts

# Counts > 100 (> 10% aggregated events frequency) - too common ??
sum(counts_per_gene > 100) # 692 - include anyway
# all are early onset BC, weighting will take care of the "common" component?

# Counts < 10 (< 1% aggregated events frequency) - too rare for detection
sum(counts_per_gene < 10) # 6,946

# Genes that may be analysed
sum(counts_per_gene >= 10) # 2,171 (ATM=24; CHEK2=14) ignore the > 100?
...
save.image(paste(interim_data_folder, "s08_filter_by_variant_effect_wecare_only.RData", sep="/"))
